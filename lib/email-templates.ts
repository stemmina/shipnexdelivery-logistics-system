...
export function applyEmailTemplate(template: { subject: string; body: string }, shipment: Pick<Shipment, "receiver_name" | "tracking_number" | "status" | "current_location" | "estimated_delivery">) {
  const values = {
    ...
    current_location: shipment.current_location?.trim() || "Location updating",
...
export function renderShipmentEmailHtml(shipment: Shipment, body: string) {
  ...
  const liveLocation = shipment.current_location?.trim() || "Location updating"
